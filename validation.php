...
//check hidden value in form for uploading images
if(isset($_POST['MAX_FILE_SIZE']) && $_POST['MAX_FILE_SIZE'] == "10000000")
{ 	
	if(empty($_FILES['userfile']['tmp_name']))
	{
		$_SESSION['errors'] = "File not found. You must choose a file to upload before uploading. File must not exceed 30000kb";
	...
	$getfilesize = $_FILES['userfile']['tmp_name'];
	...
	} else 
	{
	    ...
	    {
	        $_SESSION['OK'] = "The file ". basename( $_FILES["userfile"]["name"]). " has been uploaded.";
	        $_SESSION['OK_log'] .= $_SESSION['OK'];
...
{ 
	...
	} else 
	{
		...
		if (!empty($_POST['date_of_birth'])) {
			...
			//empty because date of birth is not required
		}
